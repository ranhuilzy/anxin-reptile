...
import com.anxin.replile.comms.constants.CodeEnum;
...
* @author 冉辉<http://blog.csdn.net/jadyer>
*/
public final class FtpUtil {
    ...
    private static boolean login(String hostname, String username, String password, boolean isTextMode, int defaultTimeout, int connectTimeout, int dataTimeout){
        ...
        logger.debug("FTP服务器["+hostname+"]连接成功...");
        ...
        try {
            if(isLoginSuccess){
                logger.debug("FTP服务器["+hostname+"]登录成功...当前所在目录为" + ftpClient.printWorkingDirectory());
            }else{
                logger.error("FTP服务器["+hostname+"]登录失败...");
                return false;
            }
            //设置文件传输类型
            if(isTextMode){
    ...
    public static void logout(){
        ...
        if(null != ftpClient){
            ...
            try{
                ftpClient.logout();
                logger.debug("FTP服务器[" + ftpRemoteAddress + "]登出成功...");
            }catch (IOException e){
                logger.warn("FTP服务器[" + ftpRemoteAddress + "]登出时发生异常,堆栈轨迹如下", e);
            }finally{
                if(null!=ftpClient && ftpClient.isConnected()){
                    try {
                        ftpClient.disconnect();
                        logger.debug("FTP服务器[" + ftpRemoteAddress + "]连接释放完毕...");
                    } catch (IOException ioe) {
                        logger.warn("FTP服务器[" + ftpRemoteAddress + "]连接释放时发生异常,堆栈轨迹如下", ioe);
                    }
                }
            }
        }
